...
#function for calculating the "returns" after every step, that is, the rewards (or lack thereof) that that agent will receive with each step.
...
class PositionalMapping(nn.Module):
    ...
    This layer map continuously inputs coordinates into a higher dimensional space
    and enables the prediction to more easily approximate a higher frequency function.
...
#defining class ActorCritic which sets the rules for the environment, but also monitors it and makes changes, if necessary.
class ActorCritic(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.actor(x) #actor acts on state x and yields state y.
        probs = self.softmax(y) #gives the probabilites associated with state y.
        value = self.critic(x) #judges the action of the actor and evaluates it, yielding the value function as a result.

        return probs, value

    def get_action(self, state, deterministic=False, exploration=0.01): #function for getting the suitable action for a specific state.

        state = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(device) #unsqueeze results in the conversion of the tensor to a tension with dimesion=1
        ...
        log_prob = torch.log(probs[action_id] + 1e-9) #forms a log of probabilites according to action_id generated, and then assigns it to the variable log_prob

        return action_id, log_prob, value

    @staticmethod
    def update_ac(network, rewards, log_probs, values, masks, Qval, gamma=0.99): #updates the ActorCritic class.
