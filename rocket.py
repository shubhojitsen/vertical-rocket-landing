...
import random
import cv2
import utils #importing utils so as to make use of all the helper functions that have been defined therein.
...
class Rocket(object):
    ...
    Their reward functions are straightforward and simple.
    ...
    """
    #defining the __init__ function to initialize the class Rocket. We list all the primary properties in this function.
    def __init__(self, max_steps, task='hover', rocket_type='starship',
    ...
    def reset(self, state_dict=None): #function responsible for resetting the environment when the rocket lands or crashes
    ...
    def create_action_table(self): #creating action table
    ...
    def check_crash(self, state): #function for checking the status of the rocket crashing, that is, whether it has crashed or not.
        if self.task == 'hover':
            x, y = state['x'], state['y']
            theta = state['theta']
            crash = False
    ...
    def check_landing_success(self, state): #function for checking the status of the rocket landing, that is, whether it has landed or not.
        if self.task == 'hover':
            return False
        elif self.task == 'landing':
            x, y = state['x'], state['y']
    ...
    def calculate_reward(self, state): #function for calculating reward for the action of the rocket, if any.
    ...
    def draw_text(self, canvas, color=(255, 255, 0)): #function for putting text on the rendered window. This text shows the velocity, speed, angular inclination, simulation steps and simulation time of the rocket as it tries to land.
    ...
    #function for drawing the trajectory of the rocket's descent as it tries to land vertically.
